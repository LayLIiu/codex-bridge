...
Codex Desktop 发出的是 Responses API 请求，但国内模型（通过 CodeIsland、AiMaMi 等中转）只提供 Chat Completions API。Bridge 把两者对接起来：

```
Codex Desktop  →  localhost:8787  →  Bridge 协议转换  →  你的上游 URL  →  模型
```

- **不处理密钥** — Key 由你的中转工具（CodeIsland / AiMaMi）注入，Bridge 不碰
...
**配合 CodeIsland / AiMaMi（最常见）：**

```
Codex Desktop → Bridge (8787) → CodeIsland/AiMaMi (本地中转) → 模型
```
...
| 上游 | Base URL 示例 | 备注 |
| --- | --- | --- |
| CodeIsland / AiMaMi | `http://127.0.0.1:端口号/v1` | 本地中转，最常见 |
...
**Q: 我用 CodeIsland / AiMaMi，怎么配置？**
A: 启动 Bridge 时 `UPSTREAM_BASE_URL` 填你中转工具的本地地址（如 `http://127.0.0.1:9090/v1`），不需要填 API Key，密钥由中转工具管理。
